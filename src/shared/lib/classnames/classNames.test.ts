...
describe('classNames', () => {
    ...
    it('with mods', () => {
        ...
        expect(classNames('someClass', { hovered: true, selected: true })).toBe(expected);
    });
    it('with mods2', () => {
        const expected = 'someClass hovered';
        // @ts-ignore
        expect(classNames('someClass', { hovered: true, selected: undefined })).toBe(expected);
    });
    it('with mods and additionals', () => {
        const expected = 'someClass class1 class2 hovered selected';
        expect(
            classNames('someClass', { hovered: true, selected: true }, ['class1', 'class2']),
        ).toBe(expected);
    });
});
